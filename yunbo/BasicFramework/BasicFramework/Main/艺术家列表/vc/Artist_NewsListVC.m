...
@interface Artist_NewsListVC ()

@end

@implementation Artist_NewsListVC

- (void)viewDidLoad {
    [super viewDidLoad];
    WeakSelf(weakSelf)
    void (^loadData)(BOOL isRefresh )=^(BOOL isRefresh){
        [weakSelf loadDataWithDataType:DataTypeList_Artist andId:@"最新" scrollView:weakSelf.tableView isRefresh:isRefresh finish:^(id value) {
            if ([value boolValue]) {
